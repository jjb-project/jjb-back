...
- 초기 MVP는 `가입 -> 휴대폰 본인인증 -> 역할 선택 -> 구직자/사장님 프로필 -> 매칭 요청/수락` 흐름을 우선한다.
- 핵심 도메인은 구직자, 사장님, 역할 전환, 휴대폰 본인인증, 사업자 인증, 매칭, 양방향 평가, 노쇼 방지다.
...
- 사장님 인증은 가입 직후 강제하지 않는다. 사장님 모드 탐색은 허용하되 첫 모집 등록 또는 첫 매칭 요청 시점에 사업자 인증을 요구한다.
