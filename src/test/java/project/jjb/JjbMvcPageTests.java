...
class JjbMvcPageTests {
	...
	void mainMvcPagesRender() throws Exception {
		mockMvc.perform(get("/"))
			...
			.andExpect(content().string(org.hamcrest.Matchers.containsString("class=\"portal-home")))
			.andExpect(content().string(org.hamcrest.Matchers.containsString("바로 찾는 단기 알바")))
			.andExpect(content().string(org.hamcrest.Matchers.containsString("지역·테마별 알바")))
	...
	void formPostFlowCreatesSessionAndSavesWorkerProfile() throws Exception {
		...
		mockMvc.perform(get("/worker/home").session(session))
			...
			.andExpect(content().string(org.hamcrest.Matchers.containsString("listing-grid")))
			.andExpect(content().string(org.hamcrest.Matchers.containsString("portal-command-bar")))
		...
		mockMvc.perform(get("/boss/home").session(ownerSession))
			...
			.andExpect(content().string(org.hamcrest.Matchers.containsString("data-live-refresh")))
			.andExpect(content().string(org.hamcrest.Matchers.containsString("listing-grid")))
			.andExpect(content().string(org.hamcrest.Matchers.containsString("오늘 필요한 인재 찾기")))
